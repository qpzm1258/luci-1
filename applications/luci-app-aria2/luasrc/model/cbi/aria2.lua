...
function get_webui_path(webui)
	local openwebui = "var curWwwPath=window.document.location.href;" ..
		"var pathName=window.document.location.pathname;" ..
		"var pos=curWwwPath.indexOf(pathName);" ..
		"var localhostPath=curWwwPath.substring(0, pos);" ..
		"var webuipath=\"http:\"+localhostPath.substring(window.location.protocol.length)+\"/".. 
		webui ..
		"\";" ..
		"window.open(webuipath)"
	return openwebui
end
local openyaaw = "window.open('http://binux.github.io/yaaw/demo')"
local openaria2webui = "window.open('http://ziahamza.github.io/webui-aria2')"
local openariang = "window.open('http://ariang.mayswind.net')"

if luci.model.ipkg.installed("yaaw") then
	openyaaw =  get_webui_path("yaaw")
end

if luci.model.ipkg.installed("webui-aria2") then
	openaria2webui =  get_webui_path("webui-aria2")
end

if luci.model.ipkg.installed("aria-ng") then
	openariang =  get_webui_path("aria-ng")
end

local running=(luci.sys.call("pidof aria2c > /dev/null") == 0)
aria_ng="&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\" " .. "AriaNg" .. " \" onclick=\'" .. openariang .. "\'/>"
yaaw="&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\" " .. "yaaw" .. " \" onclick=\'" .. openyaaw .. "\'/>"
aria2_webui="&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\" " .. "aria2-webui" .. " \" onclick=\'" .. openaria2webui .. "\'/>"

if running then
	m = Map("aria2", translate("aria2"), translate("aria2 is running") .. yaaw .. aria2_webui .. aria_ng)
else
